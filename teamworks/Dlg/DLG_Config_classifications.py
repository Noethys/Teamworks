...
##        self.label_conclusion = wx.StaticText(self, -1, "Remarques...")
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour créer une nouvelle classification")))
        self.bouton_ajouter.SetSize(self.bouton_ajouter.GetBestSize())
        self.bouton_modifier.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour modifier la classification sélectionnée dans la liste")))
        self.bouton_modifier.SetSize(self.bouton_modifier.GetBestSize())
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour supprimer la classification sélectionnée dans la liste")))
...
##        grid_sizer_base.Add(self.label_conclusion, 0, 0, 0)
    ...
    def Ajouter(self):
        dlg = wx.TextEntryDialog(self, _(u"Saisissez le nom de la nouvelle classification :"), _(u"Saisie d'une nouvelle classification"), u"")
    ...
    def Modifier(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, _(u"Vous devez d'abord sélectionner une classification à modifier dans la liste."), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, _(u"Cette classification a déjà été attribuée à ") + str(len(listeDonnees)) + _(u" contrat(s).\nEtes-vous sûr de vouloir la modifier ?"), "Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
        ...
        dlg = wx.TextEntryDialog(self, _(u"Saisissez le nom de la nouvelle classification :"), _(u"Modification du nom de la classification"), varNom)
        if dlg.ShowModal() == wx.ID_OK:
            varNom = dlg.GetValue()
            dlg.Destroy()
        else:
    ...
    def Supprimer(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, _(u"Vous devez d'abord sélectionner une classification à supprimer dans la liste."), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, _(u"Vous avez déjà enregistré ") + str(len(listeDonnees)) + _(u" contrat(s) avec cette classification. \nVous ne pouvez donc pas la supprimer."), "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        txtMessage = six.text_type((_(u"Voulez-vous vraiment supprimer cette classification ? \n\n> ") + Nom))
...
class ListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    ...
    def Remplissage(self):
        ...
        self.InsertColumn(1, _(u"Classification"))
