...
class PanelGraph(wx.Panel):
    ...
    def Save_image_temp(self):
        self.canvas.print_figure(UTILS_Fichiers.GetRepTemp("grapheTemp.png"), dpi=300)
...
##        self.ctrl_tableau.InitTableau()
    ...
    def OnBoutonModeTableau(self, event):
        if self.mode_affichage == 1 :
            self.bouton_mode_tableau.SetBitmapLabel(wx.Bitmap(Chemins.GetStaticPath("Images/32x32/Tableau.png"), wx.BITMAP_TYPE_ANY))
            self.bouton_mode_graph.SetBitmapLabel(wx.Bitmap(Chemins.GetStaticPath("Images/32x32/GraphNB.png"), wx.BITMAP_TYPE_ANY))
            self.mode_affichage = 0
            self.MAJdonnees()
    ...
    def OnBoutonModeGraph(self, event):            
        if self.mode_affichage == 0 :
            self.bouton_mode_tableau.SetBitmapLabel(wx.Bitmap(Chemins.GetStaticPath("Images/32x32/TableauNB.png"), wx.BITMAP_TYPE_ANY))
            self.bouton_mode_graph.SetBitmapLabel(wx.Bitmap(Chemins.GetStaticPath("Images/32x32/Graph.png"), wx.BITMAP_TYPE_ANY))
...
##            CreationGraph()
    ...
    def OnBoutonImprimerImage(self, event):
        ...
        hauteur, largeur = A4
        cheminFichier = UTILS_Fichiers.GetRepTemp("grapheTemp.pdf")
        ...
        img = c.drawImage(UTILS_Fichiers.GetRepTemp("grapheTemp.png"), 0, 0, width=largeur, height=hauteur, preserveAspectRatio=True)
        c.save()
    ...
    def OnBoutonImprimerTableau(self, event):
        ...
        nomDoc = UTILS_Fichiers.GetRepTemp("tableauStatsTemp.pdf")
