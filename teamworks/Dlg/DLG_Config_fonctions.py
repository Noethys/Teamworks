...
class Panel(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        texteIntro = _(u"Vous pouvez ici créer, modifier ou supprimer les fonctions qui sont utilisées dans la création des\ncandidatures. Exemples : 'Animateur C.V.L.', 'Personnel de service', 'Infirmière', etc...")
...
##        self.label_conclusion = wx.StaticText(self, -1, "Remarques...")
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour créer une nouvelle fonctions")))
        self.bouton_ajouter.SetSize(self.bouton_ajouter.GetBestSize())
        self.bouton_modifier.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour modifier la fonction sélectionnée dans la liste")))
        self.bouton_modifier.SetSize(self.bouton_modifier.GetBestSize())
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour supprimer la fonction sélectionnée dans la liste")))
...
##        grid_sizer_base.Add(self.label_conclusion, 0, 0, 0)
    ...
    def Ajouter(self):
        ...
        # Sauvegarde
        listeDonnees = [("fonction",  varNom),]
        
        # Initialisation de la connexion avec la Base de données
        DB = GestionDB.DB()
        newID = DB.ReqInsert("fonctions", listeDonnees)
        DB.Close()
    ...
    def Modifier(self):
        ...
        # Vérifie que cette fonction n'est pas attribuée à une candidature
        DB = GestionDB.DB()
        req = "SELECT IDcand_fonction FROM cand_fonctions WHERE IDfonction=%d;" % varID
        DB.ExecuterReq(req)
        ...
        # Vérifie que cette fonction n'est pas attribuée à une offre d'emploi
##        DB = GestionDB.DB()
...
##            else: dlg.Destroy()
        ...
        # Initialisation de la connexion avec la Base de données
        DB = GestionDB.DB()
        DB.ReqMAJ("fonctions", listeDonnees, "IDfonction", varID)
        DB.Close()
    ...
    def Supprimer(self):
        ...
        # Vérifie que cette fonction n'est pas attribuée à un contrat
        DB = GestionDB.DB()
        req = "SELECT IDcand_fonction FROM cand_fonctions WHERE IDfonction=%d;" % ID
        DB.ExecuterReq(req)
        listeDonnees = DB.ResultatReq()
        DB.Close()
        if len(listeDonnees) != 0 :
...
##            return
        ...
        txtMessage = six.text_type((_(u"Voulez-vous vraiment supprimer cette fonction ? \n\n> ") + Nom))
        ...
        DB = GestionDB.DB()
        DB.ReqDEL("fonctions", "IDfonction", ID)
...
class ListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    ...
    def GetNbTitulaires(self):
        ...
        DB = GestionDB.DB()
        req = """SELECT IDfonction, Count(IDcandidature) AS CompteDeIDcandidature
        FROM cand_fonctions 
        GROUP BY IDfonction;
        """
        DB.ExecuterReq(req)
        ...
        dict = {}
        for IDfonction, nbrePersonne in listeNbTitulaires :
            dict[IDfonction] = nbrePersonne
        return dict
