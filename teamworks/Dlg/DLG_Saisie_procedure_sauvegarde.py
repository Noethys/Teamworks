...
class CTRL_Options(wx.Panel) :
    ...
    def __set_properties(self):
        self.check_demander.SetToolTip(wx.ToolTip(_(u"Teamworks affichera une boîte de dialogue proposant la sauvegarde. A défaut, la sauvegarde sera réalisée automatiquement sans demander l'avis de l'utilisateur.")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDsauvegarde=None):
        ...
        if self.IDsauvegarde != None :
            ...
            self.ctrl_parametres.GetPageAvecCode("sauvegarde").ctrl_nom.SetValue("Teamworks")
    ...
    def Importation(self):
        ...
        listeChamps = [
            ...
            "sauvegarde_modeles", "sauvegarde_editions", "condition_jours_scolaires", "condition_jours_vacances", "condition_heure", "condition_poste",
            "condition_derniere", "condition_utilisateur", "option_afficher_interface", "option_demander", "option_confirmation", "option_suppression",
            ]
        DB = GestionDB.DB()
        req = """SELECT %s
...
if __name__ == u"__main__":
    ...
    dialog_1 = Dialog(None, IDsauvegarde=None)
