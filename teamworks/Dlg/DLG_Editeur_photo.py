...
##        print self.source.GetSize()
    ...
    def Draw(self,dc):
        ...
        # Dessin du masque de décoration
        nomCadre = self.GetParent().GetCadreDecoration()
        if nomCadre != None and nomCadre != "" :
            self.bmpMasque = wx.Bitmap(Chemins.GetStaticPath("Images/CadresPhotos/" + nomCadre + ".png"), wx.BITMAP_TYPE_PNG)
    ...
    def ResizePhoto(self):
        ...
        if newLargeur < 1 : newLargeur = 1
        if newHauteur < 1 : newHauteur = 1
...
class Dialog(wx.Dialog):
    def __init__(self, parent, image=None, tailleCadre=(384, 384), titre=_(u"Editeur photo")):
        ...
        self.staticBox_decoration = wx.StaticBox(self, -1, _(u"Cadre de décoration"))
        listeCadres = FonctionsPerso.GetListeCadresPhotos()
        self.combobox_decoration = wx.Choice(self, -1, choices=listeCadres)
        
        self.staticBox_texte_photo = wx.StaticBox(self, -1, _(u"Texte personnalisé"))
    ...
    def GetBuffer(self, qualite=80):
        # Récupération de l'image dans le cadre de sélection
        return self.imgbox.GetBuffer(qualite)
    
    def GetCadre(self):
        if self.combobox_decoration.GetSelection() == 0 :
            return ""
        else:
            return self.combobox_decoration.GetStringSelection()
    
    def GetTexte(self):
        return self.texte_photo.GetValue()
    ...
    def SetCadreDecoration(self, nomCadre=None):
        if nomCadre != None and nomCadre != "" :
            self.combobox_decoration.SetStringSelection(nomCadre)
        else:
    ...
    def SetTexte(self, texte=u""):
        if texte != None :
            self.texte_photo.SetValue(texte)
...
if __name__ == "__main__":
    ...
    frm = Dialog(None, image="blobtest.jpg", tailleCadre=(220, 100))
    frm.ShowModal()
    app.MainLoop()
