...
class MyDialog(wx.Dialog):
    ...
    def __init__(self, parent, id=-1, categorie="", listeValeursDefaut=[], title=_(u"Sélection de filtres de liste")):
        ...
        # Boutons
        self.bouton_reinitialiser = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/BoutonsImages/Reinitialiser.png"), wx.BITMAP_TYPE_ANY))
...
class hyperlink_texte(Hyperlink):
    ...
    def ChangeValeur(self):
        ...
        dlg = DLG_Filtre_texte.MyDialog(self, nom_filtre=self.nomFiltre, titre_frame = _(u"Filtre"), texte=selection)
        if dlg.ShowModal() == wx.ID_OK:
            texte = dlg.GetTexte()
            dlg.Destroy()
            self.valeur = texte
