    ...
    def GetListeFichiersLocal(self) :
        ...
        for fichier in fichiers :
            ...
            if (self.prefixe == None and nomFichier.endswith("_TDATA.dat")) or (self.prefixe != None and nomFichier.endswith("_TDATA.dat") and nomFichier.startswith(self.prefixe)) :
                ...
                description = u""
                img = wx.Image("Images/80x80/Logo_tw.png", wx.BITMAP_TYPE_PNG)
                img = RecadreImg(img)
...
##                        pass
    ...
    def ModifierFichier(self, titre=""):
        ...
        for suffixe in ("TDATA", "TDOCUMENTS", "TPHOTOS") :
            try :
                source = _(u"Data/%s_%s.dat") % (titre, suffixe)
                destination = _(u"Data/%s_%s.dat") % (nouveauTitre, suffixe)
    ...
    def SupprimerFichier(self, titre=""):
        ...
        if self.mode == "local" :
            for suffixe in ("TDATA", "TDOCUMENTS", "TPHOTOS") :
                try :
                    os.remove(_(u"Data/%s_%s.dat") % (titre, suffixe))
                except Exception, err :
                    pass
