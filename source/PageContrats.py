...
##        self.staticBox_pieces_staticbox = wx.StaticBox(self, -1, _(u"Pièces à fournir"))
        ...
        self.bouton_contrats_ajouter = wx.BitmapButton(self, -1, wx.Bitmap("Images/16x16/Ajouter.png", wx.BITMAP_TYPE_PNG))
        self.bouton_contrats_modifier = wx.BitmapButton(self, -1, wx.Bitmap("Images/16x16/Modifier.png", wx.BITMAP_TYPE_PNG))
        self.bouton_contrats_supprimer = wx.BitmapButton(self, -1, wx.Bitmap("Images/16x16/Supprimer.png", wx.BITMAP_TYPE_PNG))
        self.bouton_signature = wx.BitmapButton(self, -1, wx.Bitmap("Images/16x16/Signature.png", wx.BITMAP_TYPE_PNG))
        self.bouton_due = wx.BitmapButton(self, -1, wx.Bitmap("Images/16x16/Due.png", wx.BITMAP_TYPE_PNG))
...
##        grid_sizer_base.Add(grid_sizer_haut, 1, wx.EXPAND, 0)
    ...
    def AjouterContrat(self):
        frame_saisieContrat = Wiz_Creation_Contrat.MyWizard(self, IDcontrat=0, IDpersonne=self.IDpersonne)
        frame_saisieContrat.Show()
    ...
    def ModifierContrat(self):
        ...
        frame_saisieContrat = Wiz_Creation_Contrat.MyWizard(self, IDcontrat=varIDcontrat, IDpersonne=self.IDpersonne)
        frame_saisieContrat.Show()
    ...
    def ImprimerContrat(self):
        ...
        index = self.list_ctrl_contrats.GetFirstSelected()
        IDcontrat = self.list_ctrl_contrats.GetItemData(index)
##        import Wiz_Edition_Contrat
##        frame_EditionContrat = Wiz_Edition_Contrat.MyWizard(self, IDcontrat=IDcontrat)
##        frame_EditionContrat.Show()
        
        # Récupère les données pour le publipostage
        import Publipostage_donnees
        dictDonnees = Publipostage_donnees.GetDictDonnees(categorie="contrat", listeID=[IDcontrat,])
    ...
    def ImprimerDUE(self):
        ...
        index = self.list_ctrl_contrats.GetFirstSelected()
        IDcontrat = self.list_ctrl_contrats.GetItemData(index)
        frame_EditionDUE = Edition_DUE.MyFrame(self, IDcontrat=IDcontrat)
...
class ListCtrl_contrats(wx.ListCtrl):
    ...
    def Remplissage(self):
        ...
        # Création des items
        index = 0
        for IDcontrat, valeurs in self.DictContrats.iteritems():
            etat = valeurs[0]
    ...
    def ColumnSorter(self, key1, key2):
        item1 = self.GetItem( self.FindItemData(-1, key1), 1).GetText()
        item2 = self.GetItem( self.FindItemData(-1, key2), 1).GetText()
