...
class Dialog(wx.Dialog):
    ...
    def RechercheAutorisation(self):
        ...
        req = "SELECT host, db, user FROM db WHERE user='%s' and db='%s' and host='%s';" % (self.nomUtilisateur, u"%s_tdata" % self.nomBase, hote)
        DB.ExecuterReq(req)
    ...
    def Importation_hotes(self):
        ...
        req = "SELECT Host FROM `mysql`.`db` WHERE Db='%s' AND User='%s';" % (u"%s_tdata" % self.nomBase, self.nomUtilisateur)
        DB.ExecuterReq(req)
