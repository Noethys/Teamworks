...
##            coordD = HeuresEnCoords(heureD)
    ...
    def Ajouter(self, selection):
        ...
        frame_SaisiePresences = SaisiePresences.Frm_SaisiePresences(None, selection, panelPlanning=self)
        frame_SaisiePresences.Show()

    def Modifier(self, IDpresence):
        """ Modifier une présence avec le form de saisie des présences """
        frame_SaisiePresences = SaisiePresences.Frm_SaisiePresences(None, IDmodif=IDpresence, panelPlanning=self)
        frame_SaisiePresences.Show()
    ...
    def OnMenuAjouter(self, event):
        frame_SaisiePresences = SaisiePresences.Frm_SaisiePresences(None, [self.ligne], panelPlanning=self)
        frame_SaisiePresences.Show()
        #event.Skip()
    ...
    def OnMenuAppliquerModele(self, event):
        ...
        frm_application = application_modele.frm_application_modele(self, selectionLignes=selectionsLignes, selectionPersonnes=selectionPersonnes, selectionDates=selectionDates )
    ...
    def SaisiePresences(self):
        ...
        frame_SaisiePresences = SaisiePresences.Frm_SaisiePresences(self, selectionsLignes)
        frame_SaisiePresences.Show()
...
##    def OnMotion_Imprimer(self, event):
##        self.boutonImprimer.SetBitmap(wx.Bitmap("Images/16x16/Imprimante_2.png", wx.BITMAP_TYPE_PNG))
##        self.txtImprimer.SetForegroundColour(wx.RED)
##        self.txtImprimer.Refresh()
##        event.Skip()
##
##    def OnLeaveWindow_Imprimer(self, event):
##        self.boutonImprimer.SetBitmap(wx.Bitmap("Images/16x16/Imprimante.png", wx.BITMAP_TYPE_PNG))
##        self.txtImprimer.SetForegroundColour(wx.BLACK)
##        self.txtImprimer.Refresh()
##        event.Skip()
##    
##    def Com_Imprimer(self, event):
##        print "----------------------- Commande imprimer ------------------------"
...
##        event.Skip()    
    ...
    def OnMotion_Options(self, event):
        self.boutonOptions.SetBitmap(wx.Bitmap("Images/16x16/Options_2.png", wx.BITMAP_TYPE_PNG))
        self.txtOptions.SetForegroundColour(wx.RED)
        self.txtOptions.Refresh()
        event.Skip()

    def OnLeaveWindow_Options(self, event):
        self.boutonOptions.SetBitmap(wx.Bitmap("Images/16x16/Options.png", wx.BITMAP_TYPE_PNG))
    ...
    def Menu_40(self, event):
        ...
        if ChoixType == 1 :
            ...
            import Impression_calendrier_mensuel
            dlg = Impression_calendrier_mensuel.MyDialog(None)
            dlg.ShowModal()      
        if ChoixType == 2 :
            # Type Planning annuel
            import Impression_calendrier_annuel
            dlg = Impression_calendrier_annuel.MyDialog(None)
            dlg.ShowModal()        
    ...
    def Menu_260(self, event):
        """ Afficher le nbre de présents """
        global heureMin, heureMax
        import Saisie_heures
        dlg = Saisie_heures.MyDialog(self, heureMin, heureMax)  
        if dlg.ShowModal() == wx.ID_OK:
...
##        print "dictPresences =", dictPresences
        ...
        import Impression_presences_graph
        Impression_presences_graph.Impression(orientation, dictCategories, dictGroupes, dictLignes, listePresences, dictPresences, coordLigne, hauteurBarre, ecartLignes, modeTexte)
...
class ImpressionPDFvTexte():
    def __init__(self, dictCategories, dictGroupes, dictLignes, listePresences, dictPresences, maxWidth, maxHeight) :
        ...
        nomDoc = "Temp/impression_planning.pdf"
