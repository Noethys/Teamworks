...
class Panel(wx.Panel):
    def __init__(self, parent, ID=-1, afficheBoutonAnnuel=True, afficheAujourdhui=True, bordHaut=0, bordBas=0, bordLateral=0):
        ...
        self.calendrier.couleurNormal = (214, 223, 255)
        ...
        if "linux" in sys.platform :
            ...
            largeurMois = 50
        self.combo_annee = wx.ComboBox(self, -1, "" , (-1, -1) , (largeurMois, -1), self.listeAnnees , wx.CB_READONLY)
...
##            bordLateral = bordLateral
        ...
        if self.afficheBoutonAnnuel == False : self.bouton_CalendrierAnnuel.Show(False)
        else: self.bouton_CalendrierAnnuel.Show(True)
    ...
    def OnBoutonAnnuel(self, event) :
        if self.calendrier.GetTypeCalendrier() == "mensuel" :
            ...
            if self.GetGrandParent().GetName() == "panel_saisiePresences_FicheInd" :
                self.GetGrandParent().GetParent().SetSize((910, 520))
                self.GetGrandParent().GetParent().Centre()
                
            # l'appel vient de la frame d'application des Modèles de présences de la fiche individuelle
            if self.GetGrandParent().GetName() == "panel_applicModele_FicheInd" :
                self.GetGrandParent().GetParent().SetSize((910, 520))
                self.GetGrandParent().GetParent().Centre()
