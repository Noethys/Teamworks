...
class Panel(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        self.barreTitre = FonctionsPerso.BarreTitre(self,  u"Les valeurs de points", u"")
        texteIntro = u"Vous pouvez ici créer, modifier ou supprimer les valeurs de points qui sont utilisés dans la création des contrats. Il vous suffit de préciser le montant et la valeur d'application."
...
##        self.label_conclusion = wx.StaticText(self, -1, "= Valeur actuelle")
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTipString(u"Cliquez ici pour créer une nouvelle valeur de points")
        self.bouton_ajouter.SetSize(self.bouton_ajouter.GetBestSize())
        self.bouton_modifier.SetToolTipString(u"Cliquez ici pour modifier la valeur de points sélectionnée dans la liste")
        self.bouton_modifier.SetSize(self.bouton_modifier.GetBestSize())
        self.bouton_supprimer.SetToolTipString(u"Cliquez ici pour supprimer la valeurs de points sélectionnée dans la liste")
...
##        grid_sizer_base.Add(self.label_conclusion, 0, 0, 0)
    ...
    def Modifier(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner une valeur à modifier dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Cette valeur de point a déjà été attribuée à " + str(len(listeDonnees)) + u" contrat(s).\nEtes-vous sûr de vouloir la modifier ?", "Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner une valeur à supprimer dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Vous avez déjà enregistré " + str(len(listeDonnees)) + u" contrat(s) avec cette valeur de point. \nVous ne pouvez donc pas la supprimer.", "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demande de confirmation
        txtMessage = unicode((u"Voulez-vous vraiment supprimer cette valeur ? \n\n> " + Nom))
...
class ListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    ...
    def Remplissage(self):
        ...
        self.InsertColumn(1, u"Valeur")
        self.SetColumnWidth(1, 80)
        self.InsertColumn(2, u"Date d'application")
...
class MyFrame(wx.Frame):
    ...
    def __set_properties(self):
        self.SetTitle(u"Gestion des valeurs de points")
        _icon = wx.EmptyIcon()
