...
class Panel(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        self.barreTitre = FonctionsPerso.BarreTitre(self,  u"Les types de contrats", u"")
        texteIntro = u"Vous pouvez ici créer, modifier ou supprimer les types de contrats qui sont utilisés\ndans la création des contrats. Exemples : 'Contrat à Durée Déterminée' ou 'Contrat à\nDurée Indéterminée', etc..."
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTipString(u"Cliquez ici pour créer un nouveau type de contrat")
        self.bouton_ajouter.SetSize(self.bouton_ajouter.GetBestSize())
        self.bouton_modifier.SetToolTipString(u"Cliquez ici pour modifier le type de contrat sélectionné dans la liste")
        self.bouton_modifier.SetSize(self.bouton_modifier.GetBestSize())
        self.bouton_supprimer.SetToolTipString(u"Cliquez ici pour supprimer le type de contrat sélectionné dans la liste")
...
##        grid_sizer_base.Add(self.label_conclusion, 0, 0, 0)
    ...
    def Ajouter(self):
        frmSaisieTypes = Saisie_types_contrats.MyFrame(self, "", IDtype=0)
        frmSaisieTypes.Show()
    ...
    def Modifier(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner un type de contrat à modifier dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Ce type de contrat a déjà été attribué à " + str(len(listeDonnees)) + u" contrat(s).\nEtes-vous sûr de vouloir le modifier ?", "Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
        ...
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Ce type de contrat a déjà été attribué à " + str(len(listeDonnees)) + u" modèle(s) de contrat.\nEtes-vous sûr de vouloir le modifier ?", "Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            if reponse == wx.ID_NO:
                dlg.Destroy()
                return
        ...
        frmSaisieTypes = Saisie_types_contrats.MyFrame(self, "", IDtype=ID)
        frmSaisieTypes.Show()
    ...
    def Supprimer(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner un type de contrat à supprimer dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Vous avez déjà enregistré " + str(len(listeDonnees)) + u" contrat(s) avec ce type de contrat. \nVous ne pouvez donc pas la supprimer.", "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Vous avez déjà enregistré " + str(len(listeDonnees)) + u" modèle(s) de contrat avec ce type de contrat. \nVous ne pouvez donc pas le supprimer.", "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demande de confirmation
        txtMessage = unicode((u"Voulez-vous vraiment supprimer ce type de contrat ? \n\n> " + Nom))
...
class ListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    ...
    def Remplissage(self):
        ...
        self.InsertColumn(1, u"Abrégé")
        self.SetColumnWidth(1, 60)
        self.InsertColumn(2, u"Nom complet")
...
class MyFrame(wx.Frame):
    ...
    def __set_properties(self):
        self.SetTitle(u"Gestion des types de contrats")
        _icon = wx.EmptyIcon()
