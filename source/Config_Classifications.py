...
class Panel(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        self.barreTitre = FonctionsPerso.BarreTitre(self,  u"Les classifications", u"")
        texteIntro = u"Vous pouvez ici créer, modifier ou supprimer les classifications qui sont\nutilisées dans la création des contrats. Exemples : 'Animateur C.V.L.',\n'Personnel de service', 'Infirmière', etc..."
...
##        self.label_conclusion = wx.StaticText(self, -1, "Remarques...")
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTipString(u"Cliquez ici pour créer une nouvelle classification")
        self.bouton_ajouter.SetSize(self.bouton_ajouter.GetBestSize())
        self.bouton_modifier.SetToolTipString(u"Cliquez ici pour modifier la classification sélectionnée dans la liste")
        self.bouton_modifier.SetSize(self.bouton_modifier.GetBestSize())
        self.bouton_supprimer.SetToolTipString(u"Cliquez ici pour supprimer la classification sélectionnée dans la liste")
...
##        grid_sizer_base.Add(self.label_conclusion, 0, 0, 0)
    ...
    def Ajouter(self):
        dlg = wx.TextEntryDialog(self, u"Saisissez le nom de la nouvelle classification :", u"Saisie d'une nouvelle classification", u"")
        ...
        DB = GestionDB.DB()
        newID = DB.ReqInsert("contrats_class", listeDonnees)
        DB.Close()
    ...
    def Modifier(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner une classification à modifier dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Vérifie que cette classification n'est pas attribuée à un contrat
        DB = GestionDB.DB()
        req = "SELECT IDcontrat FROM contrats WHERE IDclassification=%d;" % varID
        DB.ExecuterReq(req)
        listeDonnees = DB.ResultatReq()
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Cette classification a déjà été attribuée à " + str(len(listeDonnees)) + u" contrat(s).\nEtes-vous sûr de vouloir la modifier ?", "Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
        ...
        # Vérifie que cette classification n'est pas attribuée à un modèle de contrat
        DB = GestionDB.DB()
        ...
        dlg = wx.TextEntryDialog(self, u"Saisissez le nom de la nouvelle classification :", u"Modification du nom de la classification", varNom)
        if dlg.ShowModal() == wx.ID_OK:
            varNom = dlg.GetValue()
            dlg.Destroy()
        else:
        ...
        # Initialisation de la connexion avec la Base de données
        DB = GestionDB.DB()
        DB.ReqMAJ("contrats_class", listeDonnees, "IDclassification", varID)
        DB.Close()
    ...
    def Supprimer(self):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, u"Vous devez d'abord sélectionner une classification à supprimer dans la liste.", "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Vérifie que cette classification n'est pas attribuée à un contrat
        DB = GestionDB.DB()
        req = "SELECT IDcontrat FROM contrats WHERE IDclassification=%d;" % ID
        DB.ExecuterReq(req)
        listeDonnees = DB.ResultatReq()
        DB.Close()
        if len(listeDonnees) != 0 :
            dlg = wx.MessageDialog(self, u"Vous avez déjà enregistré " + str(len(listeDonnees)) + u" contrat(s) avec cette classification. \nVous ne pouvez donc pas la supprimer.", "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        txtMessage = unicode((u"Voulez-vous vraiment supprimer cette classification ? \n\n> " + Nom))
        ...
        DB = GestionDB.DB()
        DB.ReqDEL("contrats_class", "IDclassification", ID)
...
class ListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    ...
    def Remplissage(self):
        ...
        self.InsertColumn(1, u"Classification")
    ...
    def GetNbTitulaires(self):
        ...
        DB = GestionDB.DB()
        req = """SELECT IDclassification, Count(IDcontrat) AS CompteDeIDcontrat
        FROM contrats 
        GROUP BY IDclassification;
        """
        DB.ExecuterReq(req)
        ...
        dict = {}
        for IDclassification, nbrePersonne in listeNbTitulaires :
            dict[IDclassification] = nbrePersonne
        return dict
...
class MyFrame(wx.Frame):
    ...
    def __set_properties(self):
        self.SetTitle(u"Gestion des classifications")
        _icon = wx.EmptyIcon()
